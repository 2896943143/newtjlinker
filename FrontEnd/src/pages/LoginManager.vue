<template>
  <div class="page-container" >
    ...
    <p class="role-label" style="font-size: 24px; margin-right: 00px; margin-top: -40px; margin-bottom: 40px;  font-weight: 600">管理员登录</p>
    ...
    <div class='button-container' style="margin-top: -10px">
      <ShadowButton class="fancy-button" width='221px' height='60px' style='margin-left: 50px' @click="handleLogin">
        <p style='font-size: 24px; margin-right: 10px'>登录</p>
      </ShadowButton>



      <div style="display: flex; justify-content: space-between; margin-left: 50px; width: 90%; margin-top: 15px;">
...
const handleLogin = async () => {
  ...
  const url = 'http://127.0.0.1:8000/api/admin_login/';
  try {
    ...
    if (data.success) {
      ...
      localStorage.setItem('AdminId', input_id.value)
      router.push('/manager')
    } else {
      inputError.value.id = true;
      inputError.value.password = true;
